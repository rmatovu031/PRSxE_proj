...
def main():
    ...
    input_path = os.getenv("INPUT_FILE", "/users/rmatovu/proj_GxE/dataset_with_nan.csv")
    output_dir = os.getenv("OUTPUT_DIR", "/users/rmatovu/proj_GxE/results_eur/")
    os.makedirs(output_dir, exist_ok=True)

    df = pd.read_csv(input_path, sep="\t")
    ## European dataset
    df_EUR = df[df["pop"] == "EUR"]

    # STEP2: case-control balance
    case_count = df_EUR[df_EUR['t2d_cc'] == 1].shape[0]
    control_count = df_EUR[df_EUR['t2d_cc'] == 0].shape[0]

    if control_count > case_count:
        cases = df_EUR[df_EUR['t2d_cc'] == 1]
        controls = df_EUR[df_EUR['t2d_cc'] == 0].sample(n=case_count, random_state=42)
        df_eur = pd.concat([cases, controls], axis=0)
        df_eur = df_eur.sample(frac=1, random_state=42).reset_index(drop=True)
        
        print(f"New balanced counts - Cases: {df_eur[df_eur['t2d_cc'] == 1].shape[0]}, "
            f"Controls: {df_eur[df_eur['t2d_cc'] == 0].shape[0]}")
    else:
        df_eur = df_EUR.copy()
        print("Controls are already equal or fewer than cases - no downsampling needed")

    # STEP3: Feature distribution
    numeric_cols = df_eur.select_dtypes(include=[np.number]).columns
    n_cols = 3
    ...
    for i, column in enumerate(numeric_cols):
        sns.histplot(df_eur[column], kde=True, ax=axes[i])
        axes[i].set_title(f'Distribution of {column}')

    for j in range(i + 1, len(axes)):
        fig.delaxes(axes[j])

    plt.title("Feature Distribution for sampled European participants in the UK Biobank", fontsize=12, pad=20)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir,"eur_features_distributions.png"), dpi=300, bbox_inches='tight', facecolor='white')
    plt.close() 

    # STEP4: Training the classifier model
    X_eur = df_eur.drop(columns=["IID", "t2d_cc","pop", "weight", "height"])
    y_eur = df_eur["t2d_cc"]

    X_eur_A, X_eur_B, y_eur_A, y_eur_B = train_test_split(X_eur, y_eur, test_size=0.2, stratify=y_eur, random_state=42)
    print("Using default parameters and proceeding with analysis")
    
    # Using best parameters already obtained from bayesian optimisation)

    best_params = {
        'n_estimators': 751,
        'max_depth': 6,
        'learning_rate': 0.014149260924605491,
        'subsample': 0.7473483381138774,
        'colsample_bytree': 0.6349665858537616,
        'gamma': 2.6471719980467916,
        'min_child_weight': 12,
        'reg_alpha': 5.962679885652543,
        'reg_lambda': 6.04527781881217,
        'eval_metric': 'auc',
    ...
    best_clf.fit(X_eur_A, y_eur_A)

    # STEP5: Using SHAP to rank the features
    explainer_A = shap.Explainer(best_clf)
    shap_values_A = explainer_A(X_eur_A)

    shap.plots.bar(shap_values_A, max_display=None, show=False)
    plt.title("Top Predictors of Type 2 Diabetes in Europeans in the UK Biobank", fontsize=12, pad=20)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "top_features_eur.png"), dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()

    # STEP6: Ranking feature interactions
    explainer = shap.TreeExplainer(best_clf)
    shap_interactions = explainer.shap_interaction_values(X_eur_A)
    mean_interactions = np.abs(shap_interactions).mean(axis=0)

    prs_col = "prs_t2d"
    prs_idx = list(X_eur_A.columns).index(prs_col)
    ...
    features = X_eur_A.columns[indices]
    ...
    plt.savefig(os.path.join(output_dir, "top_interactions_eur.png"), dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    ...
    results = permutation_test_cv(X_eur_A, y_eur_A, prs_col, best_params)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "permutation_test_results_eur.png"), dpi=300, bbox_inches='tight', facecolor='white')
    plt.close() 

    ## STEP8. Model Validation

    # Get predicted class and probabilities
    y_pred = best_clf.predict(X_eur_B)
    y_proba = best_clf.predict_proba(X_eur_B)[:, 1]  # Probabilities for class 1

    # Accuracy
    accuracy = accuracy_score(y_eur_B, y_pred)
    print(f"Optimized Model Accuracy: {accuracy:.4f}")

    # Classification report
    print("\nClassification Report:\n", classification_report(y_eur_B, y_pred))

    # Confusion matrix
    print("\nConfusion Matrix:\n", confusion_matrix(y_eur_B, y_pred))

    # ROC Curve and AUC
    fpr, tpr, _ = roc_curve(y_eur_B, y_proba)
    roc_auc = auc(fpr, tpr)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "ROC_curve_eur.png"), dpi=300, bbox_inches='tight', facecolor='white')
    plt.close() 

    # Precision-Recall Curve and AUPR
    precision, recall, _ = precision_recall_curve(y_eur_B, y_proba)
    aupr = average_precision_score(y_eur_B, y_proba)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "PR_AUPR_curve_eur.png"), dpi=300, bbox_inches='tight', facecolor='white')
    plt.close() 
    ...
    for env_key, env_name in env_features.items():
        ...
        # Set a dynamic title
        plt.title(f"PRS - {env_name} Interaction for Type 2 Diabetes in Europeans", fontsize=12, pad=20)
    
        # Adjust layout & save
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"prs_{env_key}_eur.png"), dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()  # Free memory
    
        print(f" :) Saved: prs_{env_key}_eur.png")
